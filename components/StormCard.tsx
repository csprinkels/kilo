  ...
  return (
    <section className="mt-5 space-y-3" aria-label="Storm tracker">
      {storms.map((s) => {
        ...
        return (
          <Link key={s.id} href="/storms/" className="block overflow-hidden rounded-card border border-line bg-surface transition-colors hover:border-ink-2">
            <div className="flex items-start justify-between gap-3 p-4 pb-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="rounded-full px-2.5 py-0.5 text-micro font-semibold text-white" style={{ background: catColor(s.windKt, s.cls) }}>{cat.level > 0 ? `Cat ${cat.level}` : cat.short}</span>
