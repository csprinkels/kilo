...
function VolcanoBody({ onRetry }: { onRetry: () => void }) {
  ...
  // The meter runs to the worst monitor on the island, and it has to be found BEFORE the list is
  // trimmed for display: the trim takes feed order, so a sixth monitor reading unhealthy would
  ...
  // Three different reasons to open this page — the air you are breathing, what the observatory
  // says, a look at the crater — and each chip is spelled out only when its card has something to
  // show. "Vog" with no monitor on your island filters the page down to a sentence saying so, so
  // it is gated on the readings, not on the payload having arrived.
  //
  // The hero and the official USGS wording stay outside show(); the air card leaves it the moment
  // the air is bad, because at category 2 it stops being a section and becomes the only place on
  // the page that says to stay inside. And when the air or the volcano changes level the filter
  // drops back to All, so nobody reads new instructions through an old chip.
  const { bar, show, only } = usePageFilter([
    ...(air.length > 0 ? [{ id: "vog", label: "Vog" }] : []),
    ...(k ? [{ id: "observatory", label: "Observatory" }] : []),
    ...(d && d.cams.length > 0 ? [{ id: "camera", label: "Crater camera" }] : []),
  ], { clearOn: `${k?.level ?? ""}${k?.erupting ? "-erupting" : ""} ${ml?.level ?? ""} ${worst}` });

  // Picking the camera chip is asking for the photo, not for a button that asks again.
  const openCam = cam ?? (only === "camera" ? d?.cams[0]?.id ?? null : null);
  const camName = d?.cams.find((c) => c.id === openCam)?.name;
