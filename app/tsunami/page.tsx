...
const ISLAND_BOX: Record<IslandId, number[]> = { hawaii: [18.85, 20.33, -156.15, -154.73], maui: [20.45, 21.3, -157.4, -155.9], oahu: [21.2, 21.77, -158.35, -157.58], kauai: [21.75, 22.3, -160.3, -159.22] };
...
export default function TsunamiPage() {
  ...
  const lookup = () => {
    ...
    navigator.geolocation.getCurrentPosition(async ({ coords }) => {
      ...
      try {
        // The zone packs for all four islands ship inside the app, so this answer is always computed here,
        // on the phone. There used to be a fallback that posted the exact coordinates to the state GIS
        // service whenever no pack matched — which is every person standing outside Hawaiʻi, i.e. an App
        // Review tester — contradicting the promise printed in the permission dialog itself. Worse, it
        // never checked res.ok, and ArcGIS answers errors with HTTP 200 and an {error} body, so a service
        // failure read as "not in an evacuation zone": a false all-clear in a tsunami app.
        const here = (Object.entries(ISLAND_BOX) as [IslandId, number[]][]).find(([, b]) => p.lat > b[0] && p.lat < b[1] && p.lon > b[2] && p.lon < b[3])?.[0];
        if (!here) { setResult({ error: "This check only covers Hawaiʻi, Maui, Oʻahu and Kauaʻi. Check the map below instead." }); return; }
        setPos(p);   // only once we know it is on a mapped island: off-island, nothing centres a map on their block
        const pack = here === zoneIsland ? zones : await fetch(`/zones/${here}.json`).then((r) => (r.ok ? (r.json() as Promise<ZoneCollection>) : null)).catch(() => null);
        if (!pack) { setResult({ error: "Still loading the zone map for that island. Try again in a moment." }); return; }
        const z = zoneAt(p.lat, p.lon, pack);
        // A 3 km fix against ~200 m zones cannot claim to know which side of the line you are on.
        // Clamped at both ends: 150 m is the mapping tolerance, and beyond ~1.5 km "right at the edge" stops
        // meaning anything — a Wi-Fi fix would otherwise mark a town 18 km inland as borderline.
        setResult({ zone: z.code, edge: z.edgeM < Math.min(1500, Math.max(150, coords.accuracy)), offline: !navigator.onLine });
