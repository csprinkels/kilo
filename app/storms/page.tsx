...
export default function StormsPage() {
  ...
  return (
    <main className="relative z-[1] mx-auto w-full max-w-2xl px-5 pb-20 pt-6">
      ...
      <SectionNav />
      <h1 className="display mt-4 text-display font-medium leading-[1] tracking-[-0.02em] text-ink">Storm tracker</h1>
      ...
      {snap?.data && storms.length === 0 && (
        <div className="mt-6 flex items-center gap-3 rounded-card border border-line bg-surface px-4 py-4">
          ...
          <div>
            ...
            <p className="text-label text-muted">This page wakes up when the Central Pacific Hurricane Center issues advisories. Last checked {fmtDateTime(snap.data.gen)} HST.</p>
          </div>
        </div>
      )}
      {storms.map((s) => <StormTracker key={s.id} storm={s} island={island} />)}
      <p className="mt-8 text-center text-micro text-muted">Data: NWS Central Pacific Hurricane Center / National Hurricane Center. Not an emergency service. {okina("Hawaiʻi")} county alerts are the official call to act.</p>
    </main>
  );
}
